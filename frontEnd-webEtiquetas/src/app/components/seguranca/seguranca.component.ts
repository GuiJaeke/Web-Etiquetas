...
export class SegurancaComponent {
  ...
  async onSubmit() {
    if (this.qtdForm.value.qtd > 30) {
      const dialogResult = await this.openDialog(`Você deseja imprimir ${this.qtdForm.value.qtd} etiquetas de Segurança?`)
      if (!dialogResult) {
        return
      }
    }
    this.etqService.segurancaETQ(this.qtdForm.value.qtd, this.setor).subscribe({})
  }
}
